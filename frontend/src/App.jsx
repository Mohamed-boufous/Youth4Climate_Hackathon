...
// const THIRTY_MIN_INTERVAL = 10000; // 10 secondes (pour tests)
const SIMULATION_INTERVAL = 5000; // 5 secondes

function App() {
  // --- États ---
  const [sensors, setSensors] = useState({ ...initialSensors, source: 'simulation' });
  // Capteurs simulés (indépendants)
  const [liveSensors, setLiveSensors] = useState({ ...initialSensors, source: 'simulation' });
  // Données manuelles (indépendantes)
  ...
  // --- Ref pour les capteurs (utilisé par le timer 30min) ---
  const sensorsRef = useRef(liveSensors);
  useEffect(() => {
    sensorsRef.current = liveSensors;
  }, [liveSensors]);

  // --- Appel API (simulation ou backend) ---
  ...
  const handleManualSubmit = (manualData) => {
    ...
    setManualSensors(dataWithSource); // n’écrase plus les capteurs simulés
    fetchDecisions(dataWithSource);
  };

  // Rafraîchir les décisions quand on change de mode
  useEffect(() => {
    if (inputMode === 'sensors') {
      fetchDecisions(liveSensors);
    } else if (inputMode === 'manual') {
      fetchDecisions(manualSensors);
    }
  }, [inputMode]); // volontairement sans dépendances supplémentaires
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-green-100 overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            {/* Radio buttons pour mode d'entrée */}
